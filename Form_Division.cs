...
namespace Quiz
{
    public partial class Form_Division : Form //форма для проверки деления
    {
        private TextBox[] textBox;
        private Label[] labels_prim;
        private PictureBox[] picture_boxes;
        private int znach = Form_Start.znach;
        private int[] division = new int[30];
        ...
        public Form_Division(int param)
        {
            this.param = param;
            InitializeComponent();
            Creates_Text_Box();
        ...
        private void InitializeComponent()
        {
            ...
            //текст который будет отображаться в шапке окна
            this.Text = "Проверка умений выполнять деления";
            this.ResumeLayout(false);
            //удаляется кнопка развернуть окно и свернуть окно
            this.MaximizeBox = false;
            this.MinimizeBox = false;
            //не дает изменять размер окна
        ...
        private void Creates_Labels()
        {
            ...
            for (int i = 0; i < labels_prim.Length; i++)
            {
                ...
                else if (i >= 20) labels_prim[i].Location = new Point(600, (i - 20) * 35 + 11);
                else labels_prim[i].Location = new Point(300, (i - 10) * 35 + 11);
        ...
        private void Creates_Text_Box()
        {
            ...
            for (int i = 0; i < textBox.Length; i++)
            {
                ...
                if (i < 10) textBox[i].Location = new Point(100, i * 35 + 12);
                else if (i >= 20) textBox[i].Location = new Point(100 + 600, (i - 20) * 35 + 12);
                else textBox[i].Location = new Point(100 + 300, (i - 10) * 35 + 12);
        ...
        string Text_Prim(int i)
        {
            ...
            first_term = random.Next(1, 10);
            second_term = random.Next(1, znach + 1);


            int rezul = first_term * second_term;

            division[i] = second_term;
            return rezul.ToString() + " / " + first_term.ToString() + " =";
        ...
        private void Creates_Picture_Box()
        {
            ...
            for (int i = 0; i < picture_boxes.Length; i++)
            {
                ...
                if (i < 10) picture_boxes[i].Location = new Point(165, i * 35 + 15);
                else if (i >= 20) picture_boxes[i].Location = new Point(165 + 600, (i - 20) * 35 + 15);
                else picture_boxes[i].Location = new Point(165 + 300, (i - 10) * 35 + 15);
        ...
        private void Proverka(int i)
        {
            ...
            if (int.TryParse(textBox[i].Text, out int rez) && division[i] == rez)
